...
func (c *cipherText) DecryptRaw(
	sk kyber.Scalar,
	group anon.Suite, domainSep []byte,
...
func DecryptRaw(
	sk kyber.Scalar,
	group anon.Suite, domainSep []byte, cipherText []*ElGamalBitPair,
) (plaintextShare kyber.Scalar, err error) {
	if len(cipherText) > plaintextMaxSizeBytes*4 {
		return nil, errors.Errorf("ciphertext too large (max %d pairs)",
			plaintextMaxSizeBytes*4,
		)
	}
